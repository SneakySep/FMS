<?php
$pageTitle = 'SwiftFreight - Customer Account Control';
$activePage = 'customers';
$chatMessage = trim(<<<'MSG'
Mabuhay! Need help managing customer accounts? Chat with us here.
MSG);
...
?>
    ...
    <main class="flex-1 flex flex-col min-w-0">
        ...
        <header class="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center">
            <h2 class="text-2xl font-black italic text-slate-900 tracking-tight">Customer Accounts</h2>
            <div class="flex-1 max-w-md mx-8">
                <div class="relative">
                    <i class="fa-solid fa-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                    <input type="text" placeholder="Search customers..." class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-blue focus:bg-white transition-all">
                </div>
            </div>
            <div class="flex items-center gap-3">
                ...
                <button onclick="openModal('newCustomerModal')" class="bg-brand-blue hover:bg-brand-darkblue text-white font-semibold text-xs px-4 py-2 rounded-xl transition-all shadow-md shadow-blue-500/20 flex items-center gap-2">+ Add Customer</button>
            </div>
        </header>
        ...
        <div class="p-8 space-y-6 max-w-7xl w-full mx-auto">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
                <div>
                    <h3 class="text-base font-extrabold text-slate-900">Portal Access Control</h3>
                    <p class="text-xs text-slate-400">Enable or disable customer access to the portal instantly</p>
                </div>
                    ...
                    <table class="w-full text-left text-xs">
                        <thead>
                            <tr class="border-b border-slate-100 text-slate-400 font-bold uppercase text-[10px] tracking-wider">
                                ...
                                <th class="pb-3">LOGIN EMAIL</th>
                                <th class="pb-3">DEFAULT WAREHOUSE</th>
                                <th class="pb-3 text-center">SHIPMENTS</th>
                                <th class="pb-3 text-center">PORTAL</th>
