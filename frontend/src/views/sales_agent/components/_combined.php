...
<!-- NOTE: All element IDs (modalLeadId, modalCompany, modalCode, modalStatusBadge, modalContact,
     modalEmail, modalPhone, modalPlatform, modalService, modalRoute, modalCargo, modalStatusSelect,
     modalPriceInput, modalPickupAddress, modalPickupDateTime, pickupFieldsSection,
     contactModalEmailBtn, contactModalEmailText, contactModalPhoneBtn, contactModalPhoneText, viewModal)
     are preserved so sales_agent/myleads.js & contact_modal.js keep working unchanged. -->
<style>
  /* Real entrance animations (the previous animate-in/zoom-in-95 utilities were not loaded) */
  @keyframes lmBackdropIn { from { opacity: 0; } to { opacity: 1; } }
  @keyframes lmPanelIn {
...
<script>
  // Additive pipeline-stepper sync. Runs alongside (never replaces) the existing openViewModal().
  (function () {
    function stageOrder(s) {
      return ['new_inquiry','qualifying','quote_sent','negotiation','closed_won','closed_lost'].indexOf(s);
    }
    function syncStepper() {
      try {
        var select = document.getElementById('modalStatusSelect');
        if (!select) return;
        var current = select.value || 'new_inquiry';
        var activeIdx = stageOrder(current);
        var steps = document.querySelectorAll('#viewModal .lm-step');
        steps.forEach(function (el) {
          var idx = stageOrder(el.getAttribute('data-stage'));
          el.classList.remove('is-active','is-done');
          if (idx === activeIdx) el.classList.add('is-active');
          else if (idx < activeIdx) el.classList.add('is-done');
        });
      } catch (e) {}
    }
    var sel = document.getElementById('modalStatusSelect');
    if (sel) sel.addEventListener('change', syncStepper);
    var modal = document.getElementById('viewModal');
    if (modal && 'MutationObserver' in window) {
      new MutationObserver(function () {
        if (!modal.classList.contains('hidden')) syncStepper();
